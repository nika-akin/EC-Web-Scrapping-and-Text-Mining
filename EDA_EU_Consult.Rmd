...
eda %>%
  group_by(time, country, coop_member_states) %>% 
  summarise(Frequency = n()) %>% 
...
test <- eda %>% dplyr:: select(country, research_innov,  skills, coop_member_states, SME, uptake_ai, private_sector, public_sector, eu_data_space, startup_finance, knowhow_transfer, excel_research, testing_fac, training_skills, lighthouse, net_centres, partner_research, partner_aiproject, benefits_ai, access_testing,equity_finance) %>% 
...
eda$ec_features <- textfeatures(eda$text, word_dims = 0,  sentiment = TRUE, normalize = FALSE, verbose = TRUE) #word2vec_dims is normalize, disabled here
