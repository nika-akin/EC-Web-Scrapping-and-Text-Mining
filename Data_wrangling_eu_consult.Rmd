...
```{r extract pdf text, warning=F, message=F, echo=FALSE}
#(II)
#unused<<<<<<<<<<<<<<<<<<<<<
...
Some submitted multiple documents, thus docs will be dublicates have to identifiy and turn into unique id values
```{r}
length(unique(tb_pdf$id))

tb_pdf[duplicated(tb_pdf$id),]
```
```{r}
tb_pdf$id <- make.unique(tb_pdf$id, sep = ".")
```


Read in the survey (as csv from website) of the 1st consult round (== consult_meta)
...
```
Join first consult crawling and survey responses (==temp)
not sure if full_join [n=**1,232** with missing text 797] or left_join[n=**1,216** with 797 ]

```{r}
consult_meta %>%
     filter(!is.na(Country)) %>%
    count(initiatives = Country, sort=TRUE) %>% 
  print(n=40)
```
...
#temp <- left_join(consult_meta, tb_pdf, by = c("Reference" = "id")) %>% 
#  as_tibble()


temp <- full_join(consult_meta, tb_pdf, by = c("Reference" = "id")) %>% 
     as_tibble()

temp %>%  
  filter(is.na(text)) #observations without pdf text
...
temp <- temp %>% 
  dplyr::rename(filename = Document,
         country = Country,
...
```{r merge the two data sets (123+304) together with a total of 422 rows, 12 var}
...
three_submission <- full_join(tidy_df1, submission, 
                              by = c("id", "time", "type", "size", "org", "register", "text", "consult_round", "person", "country"))
```
...
### Most importantly, some basic text cleaning routines of the pdf texts (=='text').

```{r}
library(tidytext) # tokenize text, remove stop words
library(tidyverse) # wrangle data, count words
library(textclean) # removing non-ascii chars

consult_text_clean <- three_submission %>%
  mutate(text = str_replace(text, "Ref. Ares", ""),
    text = str_trim(text), #trim leading and trailing white space
         ...
         text = replace_non_ascii(text)) #remove extra white space from text (e.g., line breaks)) #remove punctuation
...
###For the missing values in text I want to replace them with the values of abstract if they are not missing too.
### 

```{r}
consult_text_clean$text <- ifelse(is.na(consult_text_clean$text),
                                  consult_text_clean$abstract,
                                  consult_text_clean$text)
```

Drop missing text observations for the topic modeling 
Resulting in *N* = 857 text observations

```{r}
consult_text_clean <- consult_text_clean %>% 
  filter(!is.na(text))
```
...
gg_miss_var(consult_text_clean)
```
